...
void main() {
  group('OnboardingFlowScreen', () {
    ...
    setUp(() {
      ...
      ).thenAnswer((_) => Stream.value(onboardingCubit.state));
      when(
        () => sessionCubit.state,
      ).thenReturn(const SessionState.authenticated(onboardingComplete: false));
      when(
        () => sessionCubit.stream,
      ).thenAnswer((_) => Stream.value(sessionCubit.state));
      when(
        () => sessionCubit.refreshOnboardingStatus(),
      ).thenAnswer((_) async {});
    ...
    testWidgets('does not complete onboarding from the flow screen itself', (
      tester,
    ) async {
      when(
        () => sessionCubit.refreshOnboardingStatus(),
      ...
      verifyNever(() => sessionCubit.refreshOnboardingStatus());
    });
  });
}
