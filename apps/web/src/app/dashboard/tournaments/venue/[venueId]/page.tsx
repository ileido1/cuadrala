...
interface VenueTournamentsPageProps {
  params: { venueId: string };
}

export default function VenueTournamentsPage({ params }: VenueTournamentsPageProps) {
  ...
  const handleTabClick = (status: TournamentStatus | 'ALL') => {
    ...
    if (status !== 'ALL') {
      searchParamsLocal.set('status', status);
    }
    searchParamsLocal.set('page', '1');
    const query = searchParamsLocal.toString();
  ...
  return (
    <div className="space-y-6">
      <div className="animate-fade-in">
        ...
        <h1 className="page-heading">Torneos de la sede</h1>
        <p className="text-body mt-1">Gestiona los torneos de esta sede</p>
      </div>
      ...
      {state === 'empty' && (
        <EmptyState
          ...
          description="Los torneos de esta sede aparecerán aquí"
        />
      )}
            ...
            <div className="flex justify-center gap-2 animate-fade-in stagger-3">
              {Array.from({ length: totalPages }).map((_, i) => (
                <button
                  key={i}
                  onClick={() => {
                    const paramsLocal = new URLSearchParams(searchParams.toString());
                    paramsLocal.set('page', String(i + 1));
                    window.history.pushState(null, '', `?${paramsLocal.toString()}`);
