...
`services/api/ARCHITECTURE.md`, `openspec/changes/api-architecture-refactor/exploration.md`, y artefactos de estado del change (`proposal.md`, `tasks.md`) MUST describir el estado **actual** del código tras olas 0–6, no el AS-IS inicial (9 god services, 15 repos función, controllers con Prisma inline).
...
Al completar Wave 7 (PR1–PR6), MUST existir `openspec/changes/api-architecture-refactor/verify-report.md` con checklist firmado alineado a success criteria del proposal.
