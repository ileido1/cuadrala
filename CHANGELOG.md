...
### Added
- **Tournaments handoff fidelity (M11c-3)**: the per-match "Cargar" action on the organizer's Cuadro tab now opens `ResultEntrySheet` and submits through `TournamentScheduleCubit.submitMatchResult`; it stays disabled whenever a match side has no `userId` at all (guest-only side), across every tournament format (`apps/mobile` 1.7.25+48)
- **Tournaments handoff fidelity (M11c-2)**: the organizer's Cuadro tab now renders the SE-only advancement caption ("El ganador pasa de ronda automáticamente y a los dos les llega el resultado.", D12) verbatim whenever the tournament's format is SINGLE_ELIMINATION, hidden for round-robin/americano/unknown formats (`apps/mobile` 1.7.24+47)
- **Tournaments handoff fidelity (M11c-1)**: adds `ResultEntrySheet`, a new organizer bottom sheet that posts match results through `POST /tournaments/:id/matches/:matchId/results` (D1, never settle) with one `CountStepper` per side; guest sides (no `userId`) are excluded from the submitted scores and a backend error (e.g. 409 duplicate result) renders inline without closing the sheet (`apps/mobile` 1.7.23+46)
- **Tournaments handoff fidelity (M11b)**: the organizer's Cuadro tab now renders "Partidos de hoy" from M11a's enriched schedule fields — a single flat card listing every round's matches with an uppercase round caption, showing the recorded per-side score total for finished matches (e.g. "6-3"), a "Cargar" action for live matches, and the rejecter's name plus a "Mover" action for a rejected slot ("{name} no puede a las {hh:mm}"); both actions render disabled, wired in M11c/M11d (`apps/mobile` 1.7.22+45)
- **Tournaments handoff fidelity (M11a)**: `TournamentScheduleMatchDto` now parses S6b's enriched match state fields — `matchId`, `matchStatus`, `decision`, `rejectedByName`, `sides`, `scores` — resolved by the API via `formatParameters.{scheduleKey,roundNumber,matchNumber}`; data layer only, no UI change yet (M11b renders these in "Partidos de hoy") (`apps/mobile` 1.7.21+44)
