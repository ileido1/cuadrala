...
export class UpsertMatchResultDraftUseCase {
  constructor(
  ...
  async executeSV(_input: {
    ...
    if (LATEST === null) {
      const CREATED = await this._matchResultDraftRepository.createDraftSV({
    ...
    const EXISTING_PAYLOAD = LATEST.payload as DraftPayload;
    const SAME = payloadEqualsSV(EXISTING_PAYLOAD, NEW_PAYLOAD);
    if (!SAME) {
      await this._matchResultDraftRepository.deleteConfirmationsByDraftIdSV(LATEST.id);
    }

    await this._matchResultDraftRepository.updateDraftSV(LATEST.id, { payload: NEW_PAYLOAD, status: 'DRAFT' });

    const UPDATED = await this._matchResultDraftRepository.findLatestByMatchIdSV(_input.matchId);
    if (UPDATED === null) {
      throw new AppError('ERROR_INTERNO', 'No se pudo actualizar el borrador.', 500);
    }

    return { draft: UPDATED, created: false };
  }
}
